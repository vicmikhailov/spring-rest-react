...
- **Shared frontend conventions**:
  ...
  - Tailwind CSS v4 via `styles.css` with `@import "tailwindcss"` and custom theme tokens; no separate `tailwind.config.js` or `postcss.config.js`. Prefer utility classes over inline styles; use `cn` helper (`components/lib/utils.ts`) to merge classes.
...
- **Build & run workflows** (from `README.md`/`pom.xml`):
  ...
  - Production: `./mvnw clean package` installs Node 22.22/pnpm 10.32, runs `pnpm install`, `pnpm lint`, and `pnpm build` via frontend-maven-plugin, drops built assets into `src/main/resources/static`, then packages the Spring Boot jar.
  - Run packaged app: `java -jar target/spring-rest-react-0.0.1-SNAPSHOT.jar` (serves both API and static frontend on :8080).
- **Testing state**: Only context-load smoke tests (`src/test/java/ca/mikhailov/srr/*Tests.java`); no endpoint assertions. Add controller tests when changing payloads to keep contract stable.
